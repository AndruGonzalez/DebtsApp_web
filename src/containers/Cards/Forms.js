...
class CardsForm extends React.Component {
 state = {process: false, err: '', name: '', description: '', image: null, modal:
false}
 componentDidMount = () => {
 ...
 .then(res => this.props.history.push('/cards'))
 .catch(err => {
 console.log(err);
...
onChange={this.change}
 ...
 <button type='button' className='btn-lazyfy btn-danger'
onClick={this.toggleModal}>
 {this.state.process === 'delete' ? 'Cargando...': 'Eliminar'}
 </button>
 </div>
 <div className="col-xs-6 text-left">
 <button type='button' className='btn-lazyfy' onClick={this.state._id
